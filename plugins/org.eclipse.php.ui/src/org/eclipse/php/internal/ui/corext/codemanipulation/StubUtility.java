...
public class StubUtility {
	...
	public static final List<String> BUILDIN_TYPE = Arrays.asList(new String[] { "array", "mixed", "void", "integer",
			"int", "string", "float", "double", "bool", "boolean", "resource", "null", "NULL", "object" });

	@SuppressWarnings("unchecked")
	public static MethodDeclaration createImplementationStub(ISourceModule unit, ASTRewrite rewrite,
			ImportRewrite imports, IMethod method, boolean deferred) throws CoreException {
		...
		if (typeParams.length > 0) {
			for (int i = 0; i < typeParams.length; i++) {
				...
				if (currTypeParam.getParameterType() != null) {
					String typeName = addImports(curr.getType(), declaringNamespace, importContainers, imports);
					if (typeName != null) {
						newTypeParam.setParameterType(ast.newIdentifier(curr.getType()));
					}
				}
		...
		FullyQualifiedReference returnType = (FullyQualifiedReference) ((PHPMethodDeclaration) methodDeclaration)
				.getReturnType();
		if (returnType != null && ProjectOptions.getPHPVersion(unit).isGreaterThan(PHPVersion.PHP7_0)) {
			String returnTypeName = returnType.getName();
			if (returnType.isNullable()) {
				returnTypeName = "?" + returnTypeName;
			}
			func.setReturnType(ast.newIdentifier(returnTypeName));
			addImports(returnType.getName(), declaringNamespace, importContainers, imports);
		}
		if (!deferred) {
	...
	private static Map<String, ImportDeclaration> getImportContainer(IMethod method) throws ModelException {
		...
		ImprotContainersFinder finder = new ImprotContainersFinder(importContainers);
		method.getSourceModule().accept(finder);
		return importContainers;
	}

	private static String addImports(String typeName, String declaringNamespace,
			Map<String, ImportDeclaration> importContainers, ImportRewrite imports) {
		if (BUILDIN_TYPE.contains(typeName.toLowerCase())) {
			return typeName;
		}
	...
	private static class ImprotContainersFinder implements IModelElementVisitor {

		Map<String, ImportDeclaration> importContainers;

		public ImprotContainersFinder(Map<String, ImportDeclaration> importContainers) {
