...
public class UnimplementedMethodsCorrectionProposal extends ASTRewriteCorrectionProposal {
	...
	protected ASTRewrite getRewrite() throws CoreException {
		...
		ImportRewrite imports = createImportRewrite((Program) fTypeNode.getRoot());
		for (int i = 0; i < methods.length; i++) {
			MethodDeclaration newMethodDecl = StubUtility.createImplementationStub(getCompilationUnit(), rewrite,
					imports, methods[i], isInterface);
