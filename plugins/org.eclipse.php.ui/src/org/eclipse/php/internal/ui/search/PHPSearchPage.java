...
public class PHPSearchPage extends DialogPage implements ISearchPage, IDLTKSearchConstants {
	...
	private static class SearchPatternData {
		...
		public SearchPatternData(int searchFor, int limitTo, boolean isCaseSensitive, String pattern,
				IModelElement element, boolean includeInterpreterEnvironment) {
		...
		public SearchPatternData(int searchFor, int limitTo, String pattern, boolean isCaseSensitive,
				IModelElement element, int scope, IWorkingSet[] workingSets, boolean includeInterpreterEnvironment) {
	...
	private String[] fLimitToText = { SearchMessages.SearchPage_limitTo_declarations,
			...
			SearchMessages.SearchPage_limitTo_references, SearchMessages.SearchPage_limitTo_allOccurrences
	...
	private boolean performNewSearch() {
		...
		case ISearchPageContainer.SELECTION_SCOPE:
			...
			scope = factory.createSearchScope(modelElements, includeInterpreterEnvironment, getLanguageToolkit());
		...
		case ISearchPageContainer.SELECTED_PROJECTS_SCOPE: {
			...
			scope = factory.createProjectSearchScope(res.toArray(new String[res.size()]), includeInterpreterEnvironment,
					getLanguageToolkit());
		...
		if (data.getModelElement() != null && getPattern().equals(fInitialData.getPattern())) {
			...
			// getShell());
			querySpec = new ElementQuerySpecification(data.getModelElement(), data.getLimitTo(), scope,
					scopeDescription);
		} else {
			querySpec = new PatternQuerySpecification(data.getPattern(), data.getSearchFor(), data.isCaseSensitive(),
					data.getLimitTo(), scope, scopeDescription);
			data.setModelElement(null);
		}

		DLTKSearchQuery textSearchJob = new PHPSearchQuery(querySpec);
	...
	private void getEnclosingProjects(ArrayList<String> res, String projectName, IWorkspaceRoot root) {
		...
		if (project.exists()) {
			...
			try {
				IBuildpathEntry[] resolvedBuildpath = project.getResolvedBuildpath(true);
				for (IBuildpathEntry buildpathEntry : resolvedBuildpath) {
					if (buildpathEntry.getEntryKind() == IBuildpathEntry.BPE_PROJECT) {
						getEnclosingProjects(res, buildpathEntry.getPath().lastSegment(), root);
					}
				}
	...
	private SearchPatternData getPatternData() {
		...
		match = new SearchPatternData(getSearchFor(), getLimitTo(), pattern, fCaseSensitive.getSelection(),
				fModelElement, getContainer().getSelectedScope(), getContainer().getSelectedWorkingSets(),
	...
	private boolean isValidSearchPattern() {
		...
		return SearchPattern.createPattern(getPattern(), getSearchFor(), getLimitTo(), SearchPattern.R_EXACT_MATCH,
	...
	private void doPatternModified() {
		if (fInitialData != null && getPattern().equals(fInitialData.getPattern())
				&& fInitialData.getModelElement() != null && fInitialData.getSearchFor() == getSearchFor()) {
	...
	private void initSelections() {
		...
		if (activePart != null) {
			if (ScriptUtils.checkNature(getLanguageToolkit().getNatureId(), activePart, true)) {
				try {
		...
		if (initData == null && sel instanceof ITextSelection) {
		...
		fIncludeInterpreterEnvironmentCheckbox
				.setSelection(forceIncludeInterpreterEnvironment || initData.includesInterpreterEnvironment());
	}
	...
	private SearchPatternData determineInitValuesFrom(IModelElement element) {
		...
		if (toolkit != null && !toolkit.getNatureId().equals(getLanguageToolkit().getNatureId())) {
			return null;
		}
		DLTKSearchScopeFactory factory = DLTKSearchScopeFactory.getInstance();
		...
		case IModelElement.FIELD:
			return new SearchPatternData(FIELD, REFERENCES, true, PatternStrings.getFieldSignature((IField) element),
					element, isInsideInterpreterEnvironment);
		case IModelElement.METHOD:
			IMethod method = (IMethod) element;
			int searchFor = /* method.isConstructor() ? CONSTRUCTOR : */METHOD;
			return new SearchPatternData(searchFor, REFERENCES, true, getMethodSignature(method), element,
					isInsideInterpreterEnvironment);
		}
		return null;
	}
	...
	private IDialogSettings getDialogSettings() {
		final String pageName = getLanguageToolkit().getLanguageName() + PAGE_NAME;
	...
	private static String getMethodSignature(IMethod method) {
		return ScriptElementLabels.getDefault().getElementLabel(method,
