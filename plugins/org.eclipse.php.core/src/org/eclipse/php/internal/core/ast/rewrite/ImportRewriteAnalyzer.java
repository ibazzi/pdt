...
public final class ImportRewriteAnalyzer {
	...
	private void addExistingImports(Program root) {
		...
		UseStatement curr = (UseStatement) decls.get(0);
		...
		for (int i = 1; i < decls.size(); i++) {
			...
			UseStatement next = (UseStatement) decls.get(i);
	...
	private int getPackageStatementEndPos(Program root) {
		if (root.statements() != null && root.statements().size() != 0
				&& root.statements().get(0) instanceof NamespaceDeclaration) {
			NamespaceName packDecl = ((NamespaceDeclaration) root.statements().get(0)).getName();
